...
def main():
    ...
    print(f"Loading data from C:\\Users\\USER\\PycharmProjects\\ML01\\iris.csv...")
    ...
    r_model = knn(n_neighbors =args.k, p=args.p)
    r_model.fit(X, y)
    ry_predict = r_model.predict(X)
    acc2 = np.sum(ry_predict == y) / len(y)
    print(f"Train accuracy with sklearn: {acc2 * 100 :.2f}%")
